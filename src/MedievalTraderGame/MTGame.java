...
import MedievalTraderGame.Expeditions.Expedition;
import MedievalTraderGame.Interface.*;
...
import MedievalTraderGame.Player.*;
...
public class MTGame {
    ...
    public UiInfo playerChoice(String choice){
        ...
        switch (stage){
            ...
            case RULES, ON_THE_ARRIVAL_INFO, EXPEDITION_REPORT -> {  //empty body accepts any input and navigates to next menu
    ...
    private UiInfo onMainMenuChoice(String choice) {
        ...
        switch (choice) {
            case "1":
                stage = STAGE.TRAVEL_MENU;
                break;
            case "2":
                stage = STAGE.BUY_VENDOR;
                break;

            case "3":
                stage = STAGE.SELL_INVENTORY;
                break;

            case "4":
                stage = STAGE.HIRE_MERC;
                break;
            ...
            default:
                info.consoleAdd("Invalid choice, pick within the list, 99 to quit.");
        }
        return info;
    }
    ...
    private UiInfo onTravelMenuChoice(String choice) {
        ...
        switch (choice) {
            ...
            default -> { // will do nothing if choice is invalid
    ...
    private void tryUnlockExpeditions() {
        if(town.enterprise.playerOwnsEnterpriseHere()){
            if (farm.enterprise.playerOwnsEnterpriseHere()){
                if (castle.enterprise.playerOwnsEnterpriseHere()){
                    if (port.enterprise.playerOwnsEnterpriseHere())
    ...
    public UiInfo onExpeditionChoice(String choice, Player player){
        ...
        if(win){
            expedition.levelUp();
            player.inventory.addItem( expedition.expeditionReward() );
            player.addGold( expedition.goldReward() );
        }
    ...
    private static UiInfo printRules(){
        ...
        info.viewAdd("* Traveling costs " + Player.travleCost + " for You and 'upkeep' for each mercenary.");
    ...
    public boolean isGameOver(){
        if(stage == STAGE.MAIN_MENU && player.GameIsOver()){
